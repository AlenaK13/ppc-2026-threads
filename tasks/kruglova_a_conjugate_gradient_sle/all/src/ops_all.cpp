...
bool KruglovaAConjGradSleALL::RunImpl() {
  ...
  int q = n / mpi_size;
  int rem = n % mpi_size;
  int local_n = (rank < rem) ? q + 1 : q;
  int offset = (rank < rem) ? rank * (q + 1) : rank * q + rem;
...
#pragma omp parallel for default(none) shared(local_n, n, offset, input, p_full, local_ap)
    for (int i = 0; i < local_n; ++i) {
      double sum = 0.0;
      size_t row_idx = static_cast<size_t>(offset + i) * n;
      for (int j = 0; j < n; ++j) {
        sum += input.A[row_idx + j] * p_full[j];
      }
      local_ap[i] = sum;
    }
...
#pragma omp parallel for default(none) shared(local_n, local_p, local_r, beta)
    for (int i = 0; i < local_n; ++i) {
      double val = local_r[i] + (beta * local_p[i]);
      local_p[i] = val;
    }
    rsold = rsnew;
  }
